 ...
 * @author s_largher
 */
public abstract class QTerreniBar extends c_elab.pat.icef21.QImmobiliare {
	...
	public void init(RunawayData dataTransfer) {
		...
		if (!session.isInitialized()) {
			...
			//sql.append("LEFT OUTER JOIN CANA_immobili ON Patrimoni_immobiliari.immobile = CANA_immobili.immobile AND Patrimoni_immobiliari.ID_dichiarazione = CANA_immobili.ID_dichiarazione ");
			//sql.append(" AND "+table+".ID_domanda = CANA_immobili.ID_domanda AND "+table+".ID_dichiarazione = CANA_immobili.ID_dichiarazione ");
			sql.append("WHERE Patrimoni_immobiliari.ID_tp_uso_immobile = 'RES'  AND (Patrimoni_immobiliari.ID_tp_cat_catastale IN ('A1', 'A8', 'A9')) ");
