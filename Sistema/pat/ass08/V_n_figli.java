	...
	public double getValue() {
		...
		try {
			...
			for (int i = 1; i <= records.getRows(); i++) {
				...
				// e' nato?
				boolean nato[] = isNato(i, 1, 2008);

				// e' nel nucleo?
				boolean nel_nucleo[] = isNelNucleo(i, 1, 2008);
				
				// e' presente ovvero è nato AND nel_nucleo?
				boolean presente[] = c_elab.pat.Util_apapi.and(nato, nel_nucleo);

				// e' a carico nel 2008?
				//boolean a_carico[] = isACarico(i, 2008);
				
				// e' un minore di 18 anni presente nel nucleo?
				boolean minore18[] = isMinore(i, 1, 2008, 18, false);
