 ...
 * 
 */
public class QImmobiliCeduti extends DefComponentiDich {
	...
	protected double getValoreICIRiga(int i, boolean gratuito) throws Exception {
		...
		try {
			...
			if(tipo.equals("PR")) {
				...
				long value = 0;
				value=Usufrutto.getValoreNudaProprieta_aTermine(aNum, durata, yearRif);
	...
	protected void reset() {
		ElabContext.getInstance().resetSession( QImmobiliCeduti.class.getName(), IDdomanda );
	}

	
	protected String getDefaultQuery()
	{
		String anno_produzione_redditi = "";
		String anno_produzione_patrimoni = "";
		StringBuffer sql = new StringBuffer();
		try{
			Table t= dataTransfer.executeQuery("SELECT id_servizio FROM domande WHERE id_domanda="+IDdomanda);

			sql.append("SELECT anno_produzione_redditi_max, anno_produzione_patrimoni_max FROM R_Servizi WHERE " 
					+" id_servizio="+t.getInteger(1, 1)
					);

			t= dataTransfer.executeQuery(sql.toString());
			anno_produzione_redditi = t.getString(1, 1);
			anno_produzione_patrimoni = t.getString(1, 2);
	...
	public void init(RunawayData dataTransfer) {
		...
		ElabSession session =  ElabContext.getInstance().getSession( QImmobiliCeduti.class.getName(), IDdomanda );
