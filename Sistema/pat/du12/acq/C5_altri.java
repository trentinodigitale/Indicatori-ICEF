...
public class C5_altri extends DefComponentiDich {
	...
	public void init(RunawayData dataTransfer) {
		...
		if(!IDdomanda.startsWith("*"))
		{
			...
			//                             1                         2							3
			sql.append("SELECT 100 AS peso_reddito, Redditi_altri.importo, Redditi_altri.ID_tp_erogazione ");
		...
		else
		{
			...
			//              	             1           	2			3
			sql.append("SELECT     100 AS peso_reddito, importo, ID_tp_erogazione ");
		...
		try {
			for (int i = 1; i <= records.getRows(); i++) {
				...
				if(!idTpErogazione.equals(idTpErogazioneMinimoVitale) &&
				   !idTpErogazione.equals(idTpErogazioneSussidioStraordinario) &&
				   !idTpErogazione.equals(idTpErogazioneRedditoGaranzia) &&
				   !idTpErogazione.equals(idTpErogazioneRedditoGaranziaAPAPI_2011))
				{
