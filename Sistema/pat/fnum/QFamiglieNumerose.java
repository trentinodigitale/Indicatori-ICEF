...
import c_elab.pat.icef11.C1_aut;
import c_elab.pat.icef11.C1_dip;
import c_elab.pat.icef11.C1_pens;
import c_elab.pat.icef11.C2_agr;
import c_elab.pat.icef11.C3_imp;
import c_elab.pat.icef11.C4_part;

public abstract class QFamiglieNumerose extends ElainNode {
		
	//CAMBIAMI: va cambiata ogni anno
	protected int famNum_IDRelazioneParentela_Beneficiario = 5700;
	protected int famNum_IDRelazioneParentela_ParentePrimoGrado = 5702;
	protected double limiteImportoRedditiParentePrimoGrado = 6000;
	...
	public void init(RunawayData dataTransfer) {
		...
		if (!session.isInitialized()) {
			...
			// 		   					1         	   		 2					3					   4			5
			sql.append("SELECT FN_Dati.concepiti, FN_Dati.affidati, Doc.data_presentazione, FN_Dati.resTAA, Domande.escludi_ufficio ");
	...
	public int getNumeroAffidati()
	{
		...
		return datiFamiglieNumerose.getInteger(1, 2);
	}
	...
	public int getNumeroFigli()
	{
		...
		try {
			...
			for (int i = 1; i <= records.getRows(); i++) 
			{
	            ...
	            if(idRelazioneParentela==famNum_IDRelazioneParentela_Beneficiario)
	            {
			...
			for (int i = 1; i <= records.getRows(); i++) 
			{
	            ...
	            boolean residenteDaAlmeno3Anni = records.getBoolean(i,4);
	            
	            if(idRelazioneParentela==famNum_IDRelazioneParentela_ParentePrimoGrado && residenteDaAlmeno3Anni)
	            {
	            	if(dataNascita.getTime().getTime()>dataNascitaBeneficiario.getTime().getTime())
	            	{
						...
						if(importoRedditi<limiteImportoRedditiParentePrimoGrado)
		            	{
							nFigli++;
		            	}
	            	}	            		
