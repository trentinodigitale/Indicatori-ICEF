 ...
 *
 */
public class RDC extends ElainNode {

	private String idTpErogazioneRdC = "046";

	public RDC() {	
	}

	protected void reset() {
		ElabContext.getInstance().resetSession( RDC.class.getName(), IDdomanda );
	}

	public void init(RunawayData dataTransfer) {

		ElabSession session =  ElabContext.getInstance().getSession(  RDC.class.getName(), IDdomanda  );
	...
	public double getValue() {
		...
		try {
			for (int i = 1; i <= records.getRows(); i++) {
				...
				if(idTpErogazione.equals(idTpErogazioneRdC))
				{
