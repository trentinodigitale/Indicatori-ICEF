...
public class Merito extends QStudente {
	...
	private static double conv_naz_categoria_max=90;
	private static double conv_prov_assoluta_max=90;
	private static double conv_prov_categoria_max=80;
    ...
    public double getValue() {
		...
		try {
			...
			if(java.lang.Math.abs(records.getInteger(1, 1))>0){
				if(records.getInteger(1, 12)==1){
					conv_naz_assoluta=conv_naz_assoluta_max;
				}else if(records.getInteger(1, 12)==2){
					conv_naz_assoluta=conv_prov_assoluta_max;
				}			
			}
			if(java.lang.Math.abs(records.getInteger(1, 2))>0){
				if(records.getInteger(1, 12)==1){
					...
					conv_naz_categoria=conv_prov_categoria_max;
				}		
			}
