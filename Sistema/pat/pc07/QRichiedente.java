...
public abstract class QRichiedente extends ElainNode {
    ...
    public void init(RunawayData dataTransfer) {
		...
		if (!session.isInitialized()) {
            ...
            if (n_anni > 3) {
            	n_anni = 3;
    			System.out.println("ERRORE GRAVE: non possono essere gestiti piu' di 3 anni di contributi da c_elab.pat.pc07.QRichiedente");
            }
            ...
            for (int k = 1; k <= n_anni; k++) {
            	...
            	for (int i = 1; i <= n_record; i++) {
		            ...
		            if (servizio==9310)
		                riferimento = super.getInteger(i, 2) - 63;  //TODO generalizzare per anni successivi!! vedi PNS_tp_riferimenti
		            else if (servizio==9315)
		                riferimento = super.getInteger(i, 2) - 76;  //TODO generalizzare per anni successivi!! vedi PNS_tp_riferimenti
		            else
		            	System.out.println("ERRORE GRAVE: il servizio " + servizio + " non puo' essere gestito da c_elab.pat.pc07.QRichiedente");
		            ...
		            sql.append("(1092, 1093, 1094, 1095, 1096, 1517, 1518, 1519, 1520, 1521)");             //Figli    TODO generalizzare
		            ...
		            sql.append("(1092, 1093, 1094, 1095, 1096, 1517, 1518, 1519, 1520, 1521)");             //TODO generalizzare
		            ...
		            sql.append("WHERE PNS_assistiti.ID_relazioneParentela IN ");
		            sql.append("(1097, 1098, 1099, 1100)");           										  //TODO generalizzare
		            sql.append(" AND PNS_assistiti_dati.ID_domanda = ");
		            sql.append(IDdomanda);
		            doQuery(sql.toString());
		            for (int j = 1; j <= records.getRows(); j++) {
		            ...
		            sql.append(" FROM ((Dich_icef INNER JOIN Soggetti ON Dich_icef.ID_soggetto = Soggetti.ID_soggetto) INNER JOIN Familiari ON Dich_icef.ID_dichiarazione = Familiari.ID_dichiarazione) INNER JOIN PNS_familiari ON (Familiari.ID_dichiarazione = PNS_familiari.ID_dichiarazione) AND (Familiari.ID_domanda = PNS_familiari.ID_domanda) ");
		            sql.append("WHERE Familiari.ID_relazione_parentela IN ");
		            sql.append("(1088, 1513)");            							 //Richiedente    TODO generalizzare
		            sql.append(" AND Familiari.ID_domanda = ");
		            sql.append(IDdomanda);
		            doQuery(sql.toString());
