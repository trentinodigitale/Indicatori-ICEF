...
public abstract class QRichiedente extends ElainNode {
    ...
    public void init(RunawayData dataTransfer) {
		...
		if (!session.isInitialized()) {
            ...
            if (n_anni > 3) {
            	n_anni = 3;
    			System.out.println("ERRORE GRAVE: non possono essere gestiti piu' di 3 anni di contributi da c_elab.pat.pv07.QRichiedente");
            }
            ...
            for (int k = 1; k <= n_anni; k++) {
            	...
            	for (int i = 1; i <= n_record; i++) {
            		...
            		if (servizio==9210)
            			trimestre = super.getInteger(i,4) - 48; //TODO generalizzare per anni successivi!! vedi PNS_tp_riferimenti
            		else if (servizio==9215)
            			trimestre = super.getInteger(i,4) - 53; //TODO generalizzare per anni successivi!! vedi PNS_tp_riferimenti
            		else
            			System.out.println("ERRORE GRAVE: il servizio " + servizio + " non puo' essere gestito da c_elab.pat.pv07.QRichiedente");
            		...
            		sql.append("(1113, 1114, 1115, 1116, 1117, 1418, 1419, 1420, 1421, 1422)");             //Figli   TODO generalizzare
            		...
            		sql.append(" FROM ((Dich_icef INNER JOIN Soggetti ON Dich_icef.ID_soggetto = Soggetti.ID_soggetto) INNER JOIN Familiari ON Dich_icef.ID_dichiarazione = Familiari.ID_dichiarazione) INNER JOIN PNS_familiari ON (Familiari.ID_dichiarazione = PNS_familiari.ID_dichiarazione) AND (Familiari.ID_domanda = PNS_familiari.ID_domanda) ");
            		sql.append("WHERE Familiari.ID_relazione_parentela IN ");
            		sql.append("(1109, 1414)");            							 //Richiedente    TODO generalizzare
            		sql.append(" AND Familiari.ID_domanda = ");
            		sql.append(IDdomanda);
            		doQuery(sql.toString());
