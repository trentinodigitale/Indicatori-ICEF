...
public class Imm_strumentali0 extends ElainNode {
	...
	//CAMBIAMI: va cambiata ogni anno (Nota Luca: l'anno prossimo, ovvero ITEA12, si dovrebbe prendere il valore quota_capitale della
	//tabella Redditi_partecipazione per entrambi i servizi, quindi niente if e niente differenziazione per servizio)
	int idServizioVerifica = 13230; // per verifica requisiti
	int idServizioVerificaTione = 596003; // per verifica requisiti tione
	...
	public void init(RunawayData dataTransfer) {
		...
		else
		{
			...
			sql.append("INNER JOIN ");
			sql.append("	(SELECT  ID_dichiarazione, ID_tp_impresa, valore_ici, quota ");
			sql.append("		FROM  Redditi_partecipazione ");
			sql.append("		WHERE ID_tp_impresa NOT LIKE 'FAM' ");
			sql.append("		OR ID_tp_impresa NOT LIKE 'FA2' ");
			sql.append("	 UNION ");
			sql.append("	 SELECT  ID_dichiarazione, ID_tp_impresa, valore_ici, quota ");
			sql.append("		FROM  Redditi_partecipazione ");
			sql.append("		WHERE (ID_tp_impresa LIKE 'FAM' ");
			sql.append("		OR ID_tp_impresa NOT LIKE 'FA2') ");
			sql.append("		AND quota >= 50 ");
			sql.append("	 UNION ");
			sql.append("	 SELECT  ID_dichiarazione, ID_tp_impresa, valore_ici, 100 AS quota ");
			sql.append("		FROM  Redditi_impresa ");
