...
public class V_check extends Q_assegno {
	...
	public double getValue() {
		...
		// check=9 significa domanda non idonea poichè esclusa ufficio
		...
		{
		try {
			System.out.println("****** "+tb_dati_dom.getElement(1, tb_dati_dom.getIndexOfColumnName("escludi_ufficio")));
			//se escludo solo ANF o l'intera domanda
			if((tb_dati_dom.getElement(1, 3)!= null && c_elab.pat.Util_apapi.stringToBoolean((String) tb_dati_dom.getElement(1, 3)))
					|| (tb_dati_dom.getElement(1, tb_dati_dom.getIndexOfColumnName("escludi_ufficio"))!= null && c_elab.pat.Util_apapi.stringToBoolean((String) tb_dati_dom.getElement(1, tb_dati_dom.getIndexOfColumnName("escludi_ufficio")))))
			{		
				for (int i = 0; i < 12; i++) {
					escludi_ufficio[i] = false;
				}
				
			}else if((tb_domande_sospese!=null && tb_domande_sospese.getRows()>0)){
				
				for (int i = 0; i < 12; i++) {
					escludi_ufficio[i] = true;
				}
				
				for (int i = 1; i <= tb_domande_sospese.getRows(); i++) {
					try {
						int periodo=tb_domande_sospese.getInteger(i, 2)-1;
						if(periodo>=0 && periodo<12){
							escludi_ufficio[periodo] = false;
						}
					} catch (Exception e) {
		...
		check = c_elab.pat.Util_apapi.add(check, c_elab.pat.Util_apapi.not(escludi_ufficio), 8);
