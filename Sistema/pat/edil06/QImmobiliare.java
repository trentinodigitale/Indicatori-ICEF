	...
	protected double getValoreImmobili() {
		...
		for (int i = 1; i <= records.getRows(); i++) {
			try {
				...
				//N.B. il valore ICI è già rapportato alla quota di possesso
				// del patrimonio immobiliare (vedi query)
				...
				//System.out.println("data rif = " + yearRif + " - " + monthRif
				// + " - " + dayRif);
				...
				if (tipo.equals("PR")) {
					...
					total += Usufrutto.getValoreUsuf_aVita(aNum, theDate,
							dataRif, yearRif);
					...
					// tipo non trovato
					System.out.println("Tipo diritto n. " + tipo
							+ " non previsto nella classe Immobiliare");
					total += 0;
				}
