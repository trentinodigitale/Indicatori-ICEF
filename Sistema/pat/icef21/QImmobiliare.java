...
public class QImmobiliare extends DefComponentiDich {
	 ...
	 * calcola il valore degli immobili distringuendo tra residenza e altri
	 */
	protected double getValoreImmobiliRes( boolean residenza, boolean usaDetrazioneMaxValoreNudaProprieta ) {
		return getValoreImmobiliRes( false, residenza, usaDetrazioneMaxValoreNudaProprieta );
	}
	
	/**
	 * calcola il valore degli immobili distringuendo tra residenza e altri
	 */
	protected double getValoreImmobiliRes(boolean tutti, boolean residenza, boolean usaDetrazioneMaxValoreNudaProprieta ) {
		double total = 0.0;
		double round = 1.0;
		double aggiusta = 0.01;
		int durata;
			...
			try {
				...
				else
				{
					if ( 
						...
						String ID_tp_cat_catastale=records.getString(i,4);
						if(ID_tp_cat_catastale.equalsIgnoreCase("A1") || ID_tp_cat_catastale.equalsIgnoreCase("A8") || ID_tp_cat_catastale.equalsIgnoreCase("A9") ){
							sommaValore = true;
						}
					}
				}		
	...
	protected double getValoreImmobili(boolean tutti, boolean residenza, boolean usaDetrazioneMaxValoreNudaProprieta ) {
		...
		for (int i = 1; i <= records.getRows(); i++ ){
			try {
				...
				if(sommaValore)
				{
					//non mettere questa riga di print altrimetni il calcolo è cannato!!!!!!!
					//System.out.println("i "+i+" residenza "+residenza+"  "+getValoreICIRiga(i,residenza));
					total += getValoreICIRiga(i,residenza);
				}
			} catch(NullPointerException n) {
				System.out.println("Null pointer in " + getClass().getName() + ": " + n.toString());
	...
	public void init(RunawayData dataTransfer) {
		...
		if (!session.isInitialized()) {
			...
			if(!IDdomanda.startsWith("*"))
			{
				...
				sql.append(" order by valore_ici desc ");
