...
public class C5_altri2 extends DefComponentiDich {
	...
	public void init(RunawayData dataTransfer) {
		...
		if(!IDdomanda.startsWith("*"))
		{
			...
			//AUP 2020: attualizzazione redditi secondo semestre: prendo i canoni di locazione attiva             
            ...
            if(records!=null && records.getRows()>0) {
            	for (int i = 1; i <= records.getRows(); i++) {
                    ...
                    if("1".equals(causaEmergenza2)) {
	                	double importoMesePrec2 = records.getDouble(i, 2);
	                	double importoDueMesiPrec2 = records.getDouble(i, 3); 
	                	//(mese precedente + due mesi precedenti) / 2 (media) moltiplicato per 12 mesi, quindi moltiplico direttamente per 6
	                	patrimonioImmobiliareAttualizzato = (importoMesePrec2 + importoDueMesiPrec2) *6;	                    	
                    }
            	}
            }            	
        	...
        	sql = new StringBuffer();
            //      				  1								2									3					
            sql.append("SELECT lavoratore_stagionale, lavoratore_stagionale_percepiti, lavoratore_stagionale_percipiendi");
            sql.append(" FROM aup_familiari_attualizzata");
            sql.append(" WHERE lavoratore_stagionale = 1 and id_domanda = ");
            sql.append(IDdomanda);
            doQuery(sql.toString());            
 
            String lavoratoreStagionale = "";
            boolean almenoUnLavoratoreStagionale = false;
            if(records!=null && records.getRows()>0) {
            	for (int i = 1; i <= records.getRows(); i++) {
            		lavoratoreStagionale = records.getString(i, 1); 
                    if("1".equals(lavoratoreStagionale)) {
                    	almenoUnLavoratoreStagionale = true;
                    	double indennizziPercepiti = records.getDouble(i, 2);
                    	double indennizziPercipiendi = records.getDouble(i, 3);
                    	patrimonioImmobiliareAttualizzato = patrimonioImmobiliareAttualizzato + indennizziPercepiti + indennizziPercipiendi;	  
                    }
            	}
            }               
        	...
        	sql = new StringBuffer();
            //      	
        	if("1".equals(causaEmergenza2) /*&& !almenoUnLavoratoreStagionale*/) { //<-- è stato richiesto che le indennità dello stato vengano contate anche per lavoratori stagionali
                ...
                if(records!=null && records.getRows()>0) {
                	for (int i = 1; i <= records.getRows(); i++) { 
	                	double indennita = records.getElement(i, 1) != null ? records.getDouble(i, 1) : 0d;
			...
			if("1".equals(causaEmergenza2) && !almenoUnLavoratoreStagionale) {
				sql = new StringBuffer();
			...
			if("1".equals(causaEmergenza2) && almenoUnLavoratoreStagionale) {
				sql = new StringBuffer();
				sql.append("select coalesce(sum(consistenza), 0) from aup_patrimonio_finanziario where ID_tipo_investimento not in ('BAN', 'CCP') and semestre = 2 and id_domanda = ");
				sql.append(IDdomanda);
				try {
					Table t2 = dataTransfer.executeQuery(sql.toString());
					patrimonioFinanziarioAttualizzato = (double)t2.getDouble(1, 1);
				...
				sql = new StringBuffer();
				sql.append("select coalesce(sum(consistenza) / 3, 0) from aup_patrimonio_finanziario where ID_tipo_investimento in ('BAN', 'CCP') and semestre = 2 and id_domanda = ");
				sql.append(IDdomanda);
				try {
					Table t2 = dataTransfer.executeQuery(sql.toString());
					patrimonioFinanziarioAttualizzato = patrimonioFinanziarioAttualizzato + (double)t2.getDouble(1, 1);
				} catch (DBException e) {
					patrimonioFinanziarioAttualizzato = 0d;
					e.printStackTrace();
				}	
				
				patrimonioFinanziarioAttualizzato = patrimonioFinanziarioAttualizzato - 5000;
				if(patrimonioFinanziarioAttualizzato < 0d) {
					patrimonioFinanziarioAttualizzato = 0d;
				}
