...
public abstract class QRichiedente extends ElainNode {
	...
	//CAMBIAMI: va cambiata ogni anno - fatto 1/9/15
	int pc_IDServizio = 9380;
	int pc_IDRiferimento_gennaio = 209; //vedi mese di gennaio nella tabella PNS_tp_riferimenti per il servizio in questione
	String pc_lista_IDRelazioneParentela_figli = "25644, 25645, 25646, 25647, 25648"; //vedi tabella R_Relazioni_parentela 
	//SELECT * FROM R_Relazioni_parentela	WHERE (ID_servizio = 9380) AND (parentela IN (N'FE', 'FR', 'FC', 'AR', 'AC')) 
	/**
		SELECT *
		FROM            R_Relazioni_parentela
		WHERE        (ID_servizio = 9380) AND (parentela IN (N'Parente o affine di 1° grado', 'Parente o affine di 2° grado', 'Parente  affine di  3° o 4° grado', 'minore non affidato a tempo pieno'))
	 */
	
	String pc_lista_IDRelazioneParentela_parenti = "25649, 25650, 25651, 25652"; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
	String pc_lista_IDRelazioneParentela_richiedente = "25640"; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
