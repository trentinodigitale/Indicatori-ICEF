 ...
 * @author g_barbieri
 */
public class Invalidi extends c_elab.pat.icef18.QParticolarita {
	//NB questa classe estende la classe standard icef18.QParticolarita
	...
	public double getValue() {
		...
		try {
			for (int i = 1; i <= records.getRows(); i++) {
				if ( records.getInteger(i, 7) != AutoInv18_Params.assistitoMag 
						&& records.getInteger(i, 7) != AutoInv18_Params.assistitoMin ) {
					...
					try{
						coeff_invalidita= records.getDouble(i, 9);
					}catch(Exception e){
						coeff_invalidita=0;
					}
