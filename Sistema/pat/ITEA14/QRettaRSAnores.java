...
public abstract class QRettaRSAnores extends ElainNode {
	...
	/** inizializza la Table iAcDati con i valori letti dalla query sul DB àèòù+ì-.,^ì°§
	...
	public void init(RunawayData dataTransfer) {
		...
		if (!session.isInitialized()) {
    		...
    		sql.append("	FROM Domande INNER JOIN EPU_Dati ON Domande.ID_domanda = EPU_Dati.ID_domanda "); 
			...
			sql = new StringBuffer();
			// 		   					1         	   		         2                       
			sql.append("SELECT Domande.ID_domanda, EPU_Dati.rettaRSA_nores ");
    		sql.append(" FROM Domande INNER JOIN Familiari  ON Domande.ID_domanda = Familiari.ID_domanda "); 
    		sql.append(" INNER JOIN R_Relazioni_parentela ON Familiari.ID_relazione_parentela = R_Relazioni_parentela.ID_relazione_parentela ");
    		sql.append(" INNER JOIN EPU_Dati ON Domande.ID_domanda = EPU_Dati.ID_domanda");
    		sql.append(" WHERE (Domande.ID_domanda =");
    		sql.append(IDdomanda);
