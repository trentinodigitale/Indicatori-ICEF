...
public abstract class QRichiedente extends ElainNode {
	...
	//SELECT * 	FROM            PNS_tp_riferimenti	WHERE        (ID_servizio = 9280) AND (riferimento = N'1 trimestre')
	//SELECT * FROM R_Relazioni_parentela	WHERE (ID_servizio = 9280) AND (parentela IN (N'FE', 'FR', 'FC', 'AR', 'AC')) 
	/**
		SELECT *
		FROM            R_Relazioni_parentela
		WHERE        (ID_servizio = 9280) AND (parentela IN (N'Parente o affine di 1° grado', 'Parente o affine di 2° grado', 'Parente  affine di  3° o 4° grado', 'minore non affidato a tempo pieno'))
	 */	
	//SELECT * FROM R_Relazioni_parentela	WHERE (ID_servizio = 9280) AND (parentela IN (N'Richiedente'))	
		int pv_IDServizio = 9280;
		int pv_IDRiferimento_1Trimestre = 222; //vedi 1 trimestre nella tabella PNS_tp_riferimenti per il servizio in questione
		String pv_lista_IDRelazioneParentela_figli = "25614, 25615, 25616, 25617, 25618"; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
		String pv_lista_IDRelazioneParentela_richiedente = "25610"; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
