		...
		try 
		{
			...
			else
			{
				if(servizio==idServizioRedditoGaranzia)
				{
					...
					//inserito per permettere all'APAPI di modificare la scelta 
					//decisione presa a seguito del ricalcolo degli invalidi
					decisioneUfficio = 0.0;
					int idTpSceltaAutoIcef = datiRedditoGaranzia.getInteger(1, 15);
					//APAPI hanno scelto di forzare icef del reddito
					if(idTpSceltaAutoIcef == 1){
						decisioneUfficio = 1.0;
					}else if (idTpSceltaAutoIcef == 3){
					//APAPI ha scelto l'icef dei consumi
