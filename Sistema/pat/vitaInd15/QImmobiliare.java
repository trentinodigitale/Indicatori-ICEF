...
public class QImmobiliare extends DefComponentiDich {
	...
	protected String getDefaultQuery()
	{
		...
		try{
			Table t= dataTransfer.executeQuery("Select id_periodo,id_servizio from domande where id_domanda="+IDdomanda);


			sql.append("SELECT  Inizio_vettore FROM AC_configurazione where " 
					+" id_servizio="+t.getInteger(1, 2)
					+" and id_periodo="+t.getInteger(1, 1)+" order by Inizio_periodo asc"
					);
			...
			if(annoAC>2013){
				...
				//è fisso perceh abbiamo sbagliato nel 2012 -2013
				annoAC=2011;
			}
