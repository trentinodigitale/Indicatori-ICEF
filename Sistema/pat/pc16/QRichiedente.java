...
public abstract class QRichiedente extends ElainNode {
	...
	//CAMBIAMI: va cambiata ogni anno - fatto 3/8/16
	private int pc_IDServizio_casalinghe 	= 9391;
	private int pc_IDServizio_disoccupati 	= 9396;
	private int pc_IDServizio=-1;
	private int pc_IDRiferimento_gennaio = -1; 
	
	/**
	 * 	SELECT * FROM R_Relazioni_parentela	WHERE (ID_servizio = 9380) AND (parentela IN (N'FE', 'FR', 'FC', 'AR', 'AC')) 
	 	SELECT * FROM R_Relazioni_parentela	WHERE (ID_servizio = 9380) AND (parentela IN (N'Parente o affine di 1° grado', 'Parente o affine di 2° grado', 'Parente  affine di  3° o 4° grado', 'minore non affidato a tempo pieno'))
		SELECT * FROM R_Relazioni_parentela	WHERE (ID_servizio = 9390) AND (parentela = N'Richiedente')
		SELECT * FROM R_Relazioni_parentela	WHERE (ID_servizio = 9380) AND (parentela IN (N'Richiedente'))	
	 */
	
	String pc_lista_IDRelazioneParentela_figli = ""; //vedi tabella R_Relazioni_parentela 
	String pc_lista_IDRelazioneParentela_parenti = ""; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
	String pc_lista_IDRelazioneParentela_richiedente = ""; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
    ...
    public void init(RunawayData dataTransfer) {
		...
		if (!session.isInitialized()) {
			...
			try {
				...
				tbServizio = records;
				pc_IDServizio = tbServizio.getInteger(1, 1);
				//CAMBIAMI ANNUALMENTE
				
				if(pc_IDServizio==pc_IDServizio_casalinghe){
					pc_IDRiferimento_gennaio = 409; 
					pc_lista_IDRelazioneParentela_figli = "27944, 27945, 27946, 27947, 27948"; //vedi tabella R_Relazioni_parentela 
					pc_lista_IDRelazioneParentela_parenti = "27949, 27950, 27951, 27952"; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
					pc_lista_IDRelazioneParentela_richiedente = "27940"; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
				}else{
					pc_IDRiferimento_gennaio = 509; 
					pc_lista_IDRelazioneParentela_figli = "27924, 27925, 27926, 27927, 27928"; //vedi tabella R_Relazioni_parentela 
					pc_lista_IDRelazioneParentela_parenti = "27929, 27930, 27931, 27932"; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
					pc_lista_IDRelazioneParentela_richiedente = "27920"; //vedi tabella r_relazione_parentela per il servizio in questione, formato "id1, id2, id3, ..."
				}
            ...
            if (n_anni > 3) {
            	n_anni = 3;
    			System.out.println("ERRORE GRAVE: non possono essere gestiti piu' di 3 anni di contributi QRichiedente");
            }
